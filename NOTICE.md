...
## 二创来源（Derivative Works）

- 本项目基于 [XxHuberrr/Mineradio](https://github.com/XxHuberrr/Mineradio) v2.2.0（GPL-3.0）修改而来，原版版权归 XxHuberrr 所有。
- 本项目的酷狗概念版接口移植自 [Super-55/super_music_mineradio_basic](https://github.com/Super-55/super_music_mineradio_basic)（GPL-3.0），该二创项目的接口实现归 Super-55 所有。
- 音质探测与逐档降级策略参考了 [hoowhoami/EchoMusic](https://github.com/hoowhoami/EchoMusic)；每日 VIP 自动领取/打卡思路参考了 [YTDB0/echo-auto-vip](https://github.com/YTDB0/echo-auto-vip)。
- 本二创分支的整体修改（概念版接口接入、VIP 探测链重构、音质逐档降级、每日 VIP 自动领取、信息面板、换源播放与主页显隐等）以 GPL-3.0-only 授权发布。
