...
ipcMain.handle('kugou-lite-profile-refresh', async (event) => {
  ...
  try {
    ...
    return { ok: false, error: 'KUGOU_PROFILE_REFRESH_FAILED', message: String(error && error.message || '') };
  }
});

ipcMain.handle('kugou-lite-continue-listening', async (event, limit) => {
  if (!isTrustedMainWindowIpc(event)) return { ok: false, error: 'UNTRUSTED_SENDER' };
  return kugouLiteAccount.continueListening(Number(limit) || 30);
});
...
async function ensureLocalServerStarted() {
  ...
  localServerStartPromise = (async () => {
    ...
    try {
      ...
      console.warn('[KugouLiteAccount] initialize failed:', error && error.message);
    }
