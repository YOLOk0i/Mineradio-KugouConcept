...
test('guest requests skip the lite gateway and fall back to the public songinfo route with H5 signature', async () => {
  await withRequests(({ url }) => {
    if (url.hostname === 'm.kugou.com') {
      return { body: { status: 0, err_code: 1, error: 'not available' } };
    }
    ...
    assert(calls.every(call => call.url.hostname !== 'gateway.kugou.com'), 'guest requests must not hit the authed lite gateway');
  });
});
...
test('transport and non-JSON failures reach the official retry host instead of breaking fallback', async () => {
  await withRequests(({ url }) => {
    ...
    if (url.hostname === 'gateway.kugou.com' || url.hostname === 'tracker.kugou.com' || url.hostname === 'm.kugou.com') {
    ...
    assert(calls.filter(call => call.url.hostname !== 'tracker.kugou.com').every(call => call.url.protocol === 'https:'), 'session credentials must use HTTPS');
  });
});
