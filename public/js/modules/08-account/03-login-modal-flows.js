...
function loginProviderOfficialModeText(provider) {
  ...
  if (provider === 'kugou') return { title: '扫码', sub: '酷狗概念版 App 扫码' };
...
function updateLoginProviderUi() {
  ...
  if (desc) desc.innerHTML = isQQ
    ...
    : (isKugou
      ? '使用 <b>酷狗概念版 App</b> 扫码并确认登录，登录令牌由桌面主进程加密保存。'
    : (isQishui
  ...
  if (qqCard) {
    qqCard.style.display = isKugou ? 'none' : '';
  ...
  if (st) {
    ...
    st.textContent = isQQ
      ...
      : (isKugou
        ? (kugouLoginStatus.loggedIn ? ('已登录酷狗概念版 · ' + (kugouLoginStatus.nickname || '')) : '使用酷狗概念版 App 扫码登录')
        : (isQishui
  ...
  if (refreshBtn) {
    ...
    refreshBtn.textContent = isQishui ? (qishuiOAuthBusy ? '生成中…' : '刷新二维码') : (isQQ ? (qqWebLoginBusy ? '等待扫码…' : (qqNeedsAuthRefresh ? '重新授权' : (qqNeedsMembershipSync ? '同步会员' : (qqLoginStatus.loggedIn ? '刷新状态' : '扫码登录')))) : (isKugou ? (kugouWebLoginBusy ? '等待扫码…' : (kugouLoginStatus.loggedIn ? '刷新二维码' : '扫码登录')) : (canOpenNeteaseWeb ? (neteaseWebLoginBusy ? '等待扫码…' : '网页登录') : '刷新二维码')));
  ...
  if (typeof renderKugouClaimNode === 'function') {
    renderKugouClaimNode();
    if (isKugou && kugouLoginStatus && kugouLoginStatus.loggedIn &&
        typeof refreshKugouVipClaimStatus === 'function') {
...
async function refreshQr() {
  ...
  if (loginProvider === 'kugou') {
    ...
    setKugouLiteQrStatus('正在准备酷狗概念版扫码登录…');
...
function stopQrPoll() {
  if (typeof stopKugouLiteQrPoll === 'function') stopKugouLiteQrPoll();
  if (qrPollTimer) {
    clearInterval(qrPollTimer);
    clearTimeout(qrPollTimer);
    qrPollTimer = null;
...
async function openKugouWebLogin() {
  // 酷狗登录已切换为概念版 App 扫码，由主进程账户管理器完成并经 bridge 同步。
...
async function submitQQCookieLogin() {
  ...
  if (!cookie) {
    if (statusEl) { statusEl.textContent = isKugou ? '先粘贴酷狗概念版 cookie' : '先粘贴 QQ 音乐 cookie'; statusEl.className = 'fail'; }
    return;
  }
  ...
  try {
    ...
    if (!info || !info.loggedIn) throw new Error((info && (info.message || info.error)) || (isKugou ? '酷狗概念版会话不可用' : 'QQ 会话不可用'));
    ...
    if (statusEl) { statusEl.textContent = manualPlaybackReady ? (isKugou ? '酷狗概念版会话已保存' : qqLoginStatusText(qqLoginStatus)) : (isKugou ? '酷狗概念版账号已同步，播放授权不完整，部分歌曲可能需要重登' : 'QQ 账号已同步，播放授权不完整，部分歌曲会自动换源'); statusEl.className = 'scan'; }
    setManualCookieOpenForProvider(activeAccountProvider, false);
    offerLoginCookieExport(activeAccountProvider, info);
    setTimeout(function () {
      closeLoginModal();
      showToast((manualPlaybackReady ? (isKugou ? '酷狗概念版已登录: ' : 'QQ 音乐已登录: ') : (isKugou ? '酷狗概念版账号已同步: ' : 'QQ 账号已同步: ')) + (info.nickname || info.userId || ''));
    }, 420);
  } catch (e) {
