...
async function playLocalQueueSong(song, idx, token, firstVisualPlay, opts, resumeAt) {
  ...
  forcePlaybackControlsInteractive();
  safePlaybackStep('home-dismiss-on-play-local', function () {
    if (opts.startupAutoplay) return;
    if (typeof dismissHomePage === 'function') dismissHomePage();
    else if (typeof updateEmptyHomeVisibility === 'function') updateEmptyHomeVisibility({ forceLoad: false });
  });
...
async function playQueueAt(idx, opts) {
  ...
  try {
    ...
    try {
      ...
      forcePlaybackControlsInteractive();
      // 播放真正出声后收起主页面板：换源/自动恢复等 preserveHomeState 链路
      // 成功时主页不应继续遮挡歌词（音质切换、无缝续播、开机自动播放除外）。
      safePlaybackStep('home-dismiss-on-play', function () {
        if (opts.qualitySwitch || opts.albumGaplessHandoff || opts.startupAutoplay) return;
        if (typeof dismissHomePage === 'function') dismissHomePage();
        else if (typeof updateEmptyHomeVisibility === 'function') updateEmptyHomeVisibility({ forceLoad: false });
      });
      if (playbackProvider === 'netease' && data && data.sourceMatch) {
