  ...
  function currentItems() {
    if (userPlaylists.length || (hasAnyPlatformLogin() && myPodcastCollections.length)) {
      ...
      var items = source.map(function (pl) {
        var provider = pl.provider === 'mineradio' ? 'mineradio' : (pl.provider === 'qq' ? 'qq' : (pl.provider === 'kugou' ? 'kugou' : (pl.provider === 'qishui' ? 'qishui' : 'netease')));
        var sourceLabel = provider === 'mineradio' ? 'MR' : (provider === 'qq' ? 'QQ' : (provider === 'kugou' ? 'KG' : (provider === 'qishui' ? 'QS' : 'NE')));
        return {
