...
const APP_VERSION = process.env.MINERADIO_VERSION || APP_PACKAGE.version || '2.2.1';
...
// 酷狗概念版账户桥：概念版登录的 session 由主进程账户管理器写入 bridge，
// server.js（同一主进程内）从 bridge 读取并转为 kugou cookie。
// bridge 没有登录时保留原 cookie 文件（手动粘贴）作为兜底。
if (kugouAccountBridge.getCookie()) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/kugou/vip/claim/status') {
    try {
      ...
      sendJSON(res, { provider: 'kugou', ok: false, error: err.message, message: '读取今日畅听权益状态失败' }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/kugou/vip/claim-day') {
    try {
      ...
      sendJSON(res, await handleKugouClaimDayVip(kugouCookie, body && body.date));
    } catch (err) {
      console.error('[KugouVipClaimDay]', err);
      sendJSON(res, { provider: 'kugou', ok: false, error: err.message, message: '领取今日畅听权益失败' }, 500);
    }
    return;
  }

  if (pn === '/api/kugou/vip/ad-watch') {
    try {
      await readRequestBody(req).catch(() => ({}));
      sendJSON(res, await handleKugouAdWatch(kugouCookie));
    } catch (err) {
      console.error('[KugouAdWatch]', err);
      sendJSON(res, { provider: 'kugou', ok: false, error: err.message, message: '广告任务执行失败' }, 500);
    }
    return;
  }

  if (pn === '/api/kugou/vip/grade-report') {
    try {
      const body = await readRequestBody(req).catch(() => ({}));
      sendJSON(res, await handleKugouGradeReport(kugouCookie, body && body.minutes));
    } catch (err) {
      console.error('[KugouGradeReport]', err);
      sendJSON(res, { provider: 'kugou', ok: false, error: err.message, message: '听歌时长上报失败' }, 500);
    }
    return;
  }
