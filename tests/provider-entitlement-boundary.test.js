...
function testKugouPlaybackBoundaries() {
  ...
  // 概念版语义：VIP（含每日签到赠送）即可解锁 Hi-Res/母带，不再按普通酷狗 SVIP 规则降级
  ...
  assert.strictEqual(effectiveQuality('jymaster', {
    membershipKnown: true,
    membershipVerified: true,
    playbackReady: true,
    isVip: true,
...
async function testKugouPlaybackEntitlementBoundary() {
  ...
  await withHttpsMock(({ url }) => {
    ...
    if (parsed.pathname === '/v5/url') {
      ...
      freeAppid = parsed.searchParams.get('appid') || '';
  ...
  assert.strictEqual(freeQuality, '128', 'ordinary Kugou lite-gateway playback must request standard quality');
  assert.strictEqual(freeAppid, '3116', 'ordinary Kugou playback must be signed for the lite (concept) app');

  const vipCookie = 'userid=71003; token=vip-fixture-token; kg_mid=vip-mid';
  const vipQualities = new Set();
  await withHttpsMock(({ url }) => {
    const parsed = new URL(url);
    if (isKugouWebRoleInfoEndpoint(parsed.pathname)) return unknownKugouWebRoleInfo();
    ...
    if (parsed.pathname === '/v5/url') {
      vipQualities.add(parsed.searchParams.get('quality') || '');
  ...
  assert(vipQualities.has('flac'), 'verified Kugou VIP must retain the requested lossless quality');
  ...
  await withHttpsMock(({ url }) => {
    ...
    // 概念版端点在 web 已知结论后仍会被探测，这里返回"无结论"让其走 web 兜底。
    if (isKugouMembershipEndpoint(parsed.pathname)) {
