...
function updateSearchModeTabs() {
  ...
  if ($input) {
    $input.placeholder = searchMode === 'podcast'
      ? '搜索播客、电台...'
      : (searchMode === 'kugou' ? '搜索酷狗概念版...' : (searchMode === 'qq' ? '搜索 QQ 音乐...' : (searchMode === 'netease' ? '搜索网易云音乐...' : '搜索歌曲、歌手...')));
  }
...
function songSourceTagHtml(song, opts) {
  ...
  var key = /^(netease|qq|kugou|qishui)$/.test(String(rawKey || '')) ? String(rawKey) : songProviderKey(song);
  var label = key === 'qq' ? 'QQ' : (key === 'kugou' ? 'KG' : (key === 'qishui' ? 'QS' : 'NE'));
  if (opts.switcher) {
...
function controlSourceProviders() {
  return [
    ...
    { key: 'kugou', label: 'KG', title: '酷狗概念版' },
...
function renderControlSourceSwitcher(matches) {
  ...
  el.innerHTML =
    ...
    controlSourceProviders().map(function (provider) {
      ...
      var cleanStatus = active ? '当前' : (match ? '可切换' : (controlSourceSwitcherState.loading ? '检测中' : controlSourceIssueLabel(issue)));
      var title = active ? '当前音源' : (match ? ('切换到 ' + provider.title) : (provider.title + ': ' + controlSourceIssueLabel(issue)));
      var status = active ? '当前' : (match ? '可切换' : (controlSourceSwitcherState.loading ? '检测中' : '无匹配'));
...
function searchResultMetaText(song) {
  ...
  if (songProviderKey(song) === 'kugou' && !song.playable) bits.push('酷狗概念版播放需会话/授权');
...
function searchQueryTokens(text) {
  ...
  return raw.split(/[\s·・，。,.!?！？"“”‘’()（）【】\[\]\-_/]+/).map(function (token) {
    ...
    if (!token || /^(qq|网易云|网易云音乐|酷狗|酷狗概念版|汽水|汽水音乐)$/.test(token)) return false;
