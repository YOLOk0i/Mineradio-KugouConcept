...
- 移除 Spotify 残留代码：Spotify 接口自上游起因官方 API 配额限制无法实际使用，本次彻底移除其前后端残留代码与相关测试；服务端保留 `/api/spotify/*` 的 404 响应兜底，旧数据中的 Spotify 条目保留降级防护。
- 界面文案统一为「酷狗概念版」：账号面板、推荐标签、补登按钮、音源切换/音质降级提示等处原显示「酷狗」的文案全部更正。
- 打包自检断言同步修正，版本号 `2.2.1` → `2.2.2`。
